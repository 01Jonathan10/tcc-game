	...
	def post(self, request):
		...
		if request.data['id'] == -1:
			...
			if request.data.get('cosmetic'):
				...
				controllers.PlayerController.unequip_item(self.request.user.player, request.data.get('cat'), slot=request.data.get('slot'))
			return Response({})
...
class GetQuests(BaseView):
	def get(self, request):
		...
		data = serializers.QuestSerializer(models.Quest.objects.all().extra(select = {'player_id': player.pk}), many=True).data
...
class FinishTask(BaseView):
	def post(self, request):
		...
		t_i = models.TaskInstance.objects.filter(pk=request.data.get('pk'), task__owner=player, finished__isnull=True, disabled=False)
